...
async function runTests() {
    ...
    const hash1 = (0, index_1.sha256)('hello world');
    const hash2 = (0, index_1.sha256)('hello world');
    const hash3 = (0, index_1.sha256)('hello world!');
    ...
    console.log('\n📄 Test 3: Receipt Generation');
    const scanner = new index_1.OpenClawScan({
    ...
    console.log('\n🔐 Test 4: Signature Verification');
    const verifyResult = index_1.OpenClawScan.verify(receipt);
    ...
    console.log('\n🔍 Test 5: Hash Verification with Original Data');
    const verifyWithData = index_1.OpenClawScan.verify(receipt, 'Found 3 potential vulnerabilities in the contract...');
    ...
    const verifyWithWrongData = index_1.OpenClawScan.verify(receipt, 'This is not the original output');
    ...
    const tamperResult = index_1.OpenClawScan.verify(tamperedReceipt);
    ...
    assert(!receiptJson.includes('search for smart contract vulnerabilities'), 'Raw input is NOT in receipt');
    assert(!receiptJson.includes('Found 3 potential vulnerabilities'), 'Raw output is NOT in receipt');
    // ─── Results ────────────────────────────────────────────
    console.log('\n' + '═'.repeat(50));
