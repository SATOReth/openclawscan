...
export default function TaskPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Simulate verification delay (in production, verify signatures client-side)
  ...
  if (error || !task) {
    return (
      <div className="py-8">
        <Empty icon="✗" title="Task not found" description={error || `No task with slug "${slug}"`} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <StatusBadge ok={verified && !gaps} loading={!verified} />
          ...
          <div className="text-[10.5px] overflow-auto">
            ...
            {receipts.map((r, i) => (
              ...
              >
                ...
                <span className={`text-right transition-colors ${verified ? 'text-accent' : 'text-ghost'}`}>✓</span>
              </Link>
            ))}
          </div>
        </TBox>
      ...
      <div
        ...
        style={{
          borderColor: verified ? '#22c55e33' : '#22222233',
          background: verified ? '#22c55e06' : 'transparent',
        }}
      >
        <span
          className="w-1.5 h-1.5 transition-all"
          style={{
            background: verified ? '#22c55e' : '#333',
            boxShadow: verified ? '0 0 6px #22c55e55' : 'none',
          }}
        />
        <span className={`text-[11px] transition-colors ${verified ? 'text-accent' : 'text-dim'}`}>
          {verified
            ? gaps
              ? `⚠ ${gaps}`
              : `seq #0→#${receipts.length - 1} · no gaps · ${receipts.length}/${receipts.length} signatures valid`
